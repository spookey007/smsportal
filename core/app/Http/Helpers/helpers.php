...
{
    if (!cache()->get('general')) {
        $general                = GeneralSetting::first();
        cache()->put('general', $general);
    }
    return cache()->get('general');
}

function batchId($batchFor = 1)  //1=Email,2=SMS
{
    $general = generalSetting();
    $batchPrefix = $batchFor == 1 ? $general->email_batch_prefix  : $general->sms_batch_prefix;
    $batchId = $batchPrefix . '-' . ((Batch::max('id') ?? 0) + 1);
    return $batchId;
}
