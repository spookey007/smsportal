...
function setFormValue(data, formId) {
  ...
  Array.from(elements).forEach(element => {
    ...
    //   element.value = parseInt(data[elementName]);
    // }
  });

};

$(".per-page").on('change', function ({ target }) {
...
let perPage = url.searchParams.get('per_page') || 20;
$('.per-page').val(perPage)


 function bootstrapToggleSwitch(status){
  if (status) {
